...
def callback(obs_t, obs_tp1, action, rew, done, trajs, episode):
    if len(episode) == 20:
        trajs.append(episode)
        episode = []
    episode.append(Step(cur_state=obs_t, action=action, next_state=obs_tp1, reward=rew, done=done))
    return trajs

def play(env, transpose=True, fps=30, zoom=None, callback=callback, keys_to_action=None):
    ...
    while running:
        ...
        if env_done:
            ...
            trajs = callback(prev_obs, obs, action, rew, env_done, trajs, episode)
        if obs is not None:
            if len(obs.shape) == 2:
                obs = obs[:, :, None]
            if obs.shape[2] == 1:
